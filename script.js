...
function pageLoad(page){
  if (page=="index"){
    ...
    setRingSize();
    document.getElementById('indexContent').style='color: var(--black-bean);'
    document.getElementById('atom').style='width: var(--circle-size); height: var(--circle-size);'
    return;
  }
  else if (page == "task1"){
  ...
  document.getElementById('chapContainer').style = "color:var(--black-bean);"
...
//n = how many slides to move, - is backward
...
function showDivs(n, index ,className) {
  ...
  if (n > x.length) {slideIndexes[index] = 1}
  if (n < 1) {slideIndexes[index] = x.length}
  for (i = 0; i < x.length; i++) {
    x[i].style.display = "none";  
  }
  x[slideIndexes[index]-1].style.display = "block";  
}
